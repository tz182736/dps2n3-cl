...
import { dummy_data, Summary } from '../shared/vm-summary';
                ...
                <TableHead>
                    <TableRow>
                        ...
                        {summaries[0]?.Sale.map((sale) => ( // use the first summary's Sale array to generate the column headers
                            <TableCell key={sale.Date}>{sale.Date}</TableCell>
                        ))}
                    ...
                    {summaries.map((summary) => (
                        <TableRow key={summary.Id}>
                            ...
                            {summary.Sale.map((sale) => ( // use the summary's Sale array to generate the column values
                                <TableCell key={sale.Date}>{sale.Total}</TableCell>
                            ))}
...
const App: React.FC = () => {
    return (
        <div>
            <SummaryTable summaries={dummy_data} />
        </div>
    );
};
